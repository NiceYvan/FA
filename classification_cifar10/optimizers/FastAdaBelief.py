...
class FastAdaBelief(Optimizer):
    r"""Implements FastAdaBelief algorithm. Modified from AdaBelief in PyTorch
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-16,
        ...
        if print_change_log:
            print(Fore.RED + 'Please check your arguments if you have upgraded FastAdaBelief-pytorch from version 0.0.5.')
            print(Fore.RED + 'Modifications to default arguments:')
            default_table = tabulate([
                ['fastadabelief-pytorch=0.0.5','1e-8','False','False'],
        ...
        if self.weight_decouple:
            print('Weight decoupling enabled in FastAdaBelief')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in FastAdaBelief')
        if amsgrad:
            print('AMSGrad enabled in FastAdaBelief')
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        'FastAdaBelief does not support sparse gradients, please consider SparseAdam instead')
                amsgrad = group['amsgrad']
                ...
                if amsgrad:
                    ...
                    max_exp_avg_var = max_exp_avg_var.add_(group[(1e-2)/state['step']])
                    denom = (max_exp_avg_var/ math.sqrt(bias_correction2)).add_(group['eps'])
                else:
